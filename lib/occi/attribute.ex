defmodule OCCI.Attribute do
  ...
  def spec(name, opts) do
    ...
    |> Enum.map(fn
      ...
      {:required, required} when is_boolean(required) ->
        {:required, required}

      {:description, description} when is_binary(description) or is_list(description) ->
        {:description, description}

      {:mutable, mutable} when is_boolean(mutable) ->
        {:mutable, mutable}
