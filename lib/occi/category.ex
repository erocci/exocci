defmodule OCCI.Category do
  ...
  * `model` (alias | atom): model in which category is defined
  ...
  defmacro __using__(opts) do
    scheme = Keyword.get_lazy(opts, :scheme, fn -> raise "Missing argument: scheme" end)
    term = Keyword.get_lazy(opts, :term, fn -> raise "Missing argument: term" end)
  ...
  defmacro attribute(name, opts) do
    ast =
      quote do
        Module.put_attribute(__MODULE__, :attributes, [
          Attribute.spec(unquote(name), unquote(opts))
